...
public final class RM_R extends InstructionEncoding {
    ...
    public byte[] process(final Opcode opcode, final Operand... operands) {
        ...
        final ByteBuffer byteBuffer = ByteBuffer.size(4 + destination.getDisplacement().length);
        final boolean wideMode = destination.getSize() == OperandSize.QWORD || source.getSize() == OperandSize.QWORD;
        final byte prefix = Prefix.builder().w(wideMode).r(source.isExtended()).x(destination.isIndexExtended()).b(destination.isBaseExtended()).build();
